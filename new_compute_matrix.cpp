...
int main()
{
	...
	double **A, *x, *b, *c, T1;		// x = 1234, a location in memory, x[0] the value at memory 1234, 1235, 1236, ... 1241
	int **C;
	...
	{												   // N=4, sizeof(double*)=4, 16 bytes
		...
		A = (double **) malloc(	N * sizeof(double*));  // 在記憶體中拿N個double*記憶體, 
													   // 並且把一開始指標放在 A, A[0], A[1], A[2], ... , A[N-1]
													   // 但是 A[0], A[1], A[2], ... , A[N-1] 的值是未給定的 
		A[0] = (double *) malloc(N*N*sizeof(double));  // 在記憶體中拿 N*N 個 double 記憶體, 
													   // 並且把一開始指標放在 A[0] 
